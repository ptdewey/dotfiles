_: {
  flake.homeModules.patrick =
    ...
    let
      ...
      niriProfile = config.dotfiles.desktop.niri.profile;
      niriProfileConfig =
        if niriProfile == null then
          pkgs.writeText "niri-host.kdl" ""
        else
          config.lib.file.mkOutOfStoreSymlink "${config.home.homeDirectory}/dotfiles/nix/config/desktop/niri/profiles/${niriProfile}.kdl";
      noctaliaConfig = "${config.home.homeDirectory}/dotfiles/nix/config/desktop/noctalia/config.toml";
    in
    {
      options.dotfiles.desktop.niri.profile = lib.mkOption {
        ...
        default = null;
        description = "Host-specific Niri configuration profile.";
      };
      ...
      config = {
        ...
        xdg.configFile = lib.mkIf pkgs.stdenv.hostPlatform.isLinux {
          ...
          "niri/host.kdl".source = niriProfileConfig;
