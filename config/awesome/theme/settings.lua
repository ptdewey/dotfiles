awesome.connect_signal("widget::config", function()
    ...
    local function doGeneral()
        local function wallpaper()
            ...
            if settings.wallpaper ~= nil then
                local pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                    settings.wallpaper:gsub("~", os.getenv("HOME")),
                    500,
                    500,
                    true
                )
            ...
            local wallpaper = Gtk.FileChooserButton({
                filter = filter,
            ...
            if settings.wallpaper ~= nil then
                wallpaper:set_file(
                    Gio.File.new_for_path(settings.wallpaper:gsub("~", os.getenv("HOME")))
                )
                wallpaper:set_current_folder_file(
                    Gio.File.new_for_path(settings.wallpaper:gsub("~", os.getenv("HOME")))
                )
            else
        ...
        local function screenshot()
            ...
            local button = Gtk.FileChooserButton({
                title = "Choose Screenshot Directory",
                width = dpi(245),
                action = Gtk.FileChooserAction.SELECT_FOLDER,
            })
            button:set_current_folder_file(
                Gio.File.new_for_path(settings.shotdir:gsub("~", os.getenv("HOME")))
            )
        ...
        local grid = Gtk.Grid({
            ...
            {
                settingsEntry("Shutdown", "shutdown", "systemctl poweroff"),
                top_attach = 1,
                left_attach = 0,
            },
            {
                settingsEntry("Reboot", "reboot", "systemctl reboot"),
                top_attach = 2,
                left_attach = 0,
            },
            { settingsEntry("Font", "font", "Roboto Bold 11"), top_attach = 0, left_attach = 1 },
            {
                settingsEntry("Alt Font", "fontalt", "Roboto Italic Bold 11"),
                top_attach = 1,
                left_attach = 1,
            },
            {
                settingsEntry("Icon Font", "fonticon", "Material Icons 16"),
                top_attach = 2,
                left_attach = 1,
            },
            {
                settingsEntry("Fallback Password", "passwd", "awesomewm"),
                top_attach = 3,
                left_attach = 0,
            },
    ...
    local function doColor()
        ...
        local left = Gtk.Grid({
            ...
            {
                colorEntry("Gui Theme", "gtk", color.gtk),
                top_attach = 3,
                left_attach = 0,
                width = 4,
            },
        ...
        local right = Gtk.Grid({
            ...
            {
                colorEntry("Compositor Radius", "compradius", color.compradius),
                top_attach = 0,
                left_attach = 0,
            },
            {
                colorEntry("Compositor Offset", "compoffset", color.compoffset),
                top_attach = 1,
                left_attach = 0,
            },
            {
                colorEntry("Compositor Opacity", "compopacity", color.compopacity),
                top_attach = 2,
                left_attach = 0,
            },
    ...
    local function doTheme()
        ...
        local themeprompt =
            Gtk.Label({ label = "What should this theme be called?", halign = Gtk.Align.START })
        ...
        local wallprompt =
            Gtk.Label({ label = "What wallpaper should this theme use?", halign = Gtk.Align.START })
        local filter = Gtk.FileFilter()
        filter:add_pattern("*.png")
        filter:add_pattern("*.jpg")
        ...
        local wallpaper = nil
        local wallpaperbutton = Gtk.FileChooserButton({
            filter = filter,
            title = "Choose Wallpaper",
            hexpand = true,
            preview_widget = preview,
        ...
        function wallpaperbutton:on_update_preview()
            pixbuf =
                GdkPixbuf.Pixbuf.new_from_file_at_scale(self:get_preview_filename(), 500, 500, true)
            preview:set_from_pixbuf(pixbuf)
        ...
        local colorscheme = Gtk.Grid({
            ...
            {
                Gtk.Label({ label = "Color Scheme", halign = Gtk.Align.START }),
                top_attach = 0,
                left_attach = 0,
            },
        ...
        function confirmbutton:on_clicked()
            ...
            elseif nameset == true and wallpaper then
                local themedir = require("gears").filesystem.get_configuration_dir()
                    .. "color/"
                    .. name
                    .. "/"
                ...
                color.wall = targetpath
                    :get_path()
                    :gsub(require("gears").filesystem.get_configuration_dir(), "")
    ...
    function app:on_startup()
        ...
        local saveButton =
            Gtk.Button({ label = "Save", halign = Gtk.Align.END, margin_end = dpi(20) })
        ...
        function saveButton:on_clicked()
            ...
            require("beautiful").init(
                require("gears").filesystem.get_configuration_dir() .. "theme/theme.lua"
            )
