...
if [[ "${VCS}" == "jj" ]]; then
	change_summary=$(jj diff --summary -- .) || exit
	has_changes=$([[ -n "${change_summary}" ]] && echo true || echo false)
else
